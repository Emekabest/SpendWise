...
const MonthlyBudgetScreen = () => {
  ...
  const [alterDate, setAlterDate] = useState(getOneMonthFromNow());
  const [showDatePicker, setShowDatePicker] = useState(false);
  const [dayDifference, setDayDifference] = useState(30); // Approximate
  ...
  const getMarkedDates = () => {
    ...
    // Then, enable only the dates that fall on monthly intervals
    const firstSelectableDate = getOneMonthFromNow();
    for (let m = 0; m < 12; m++) { // Enable for the next 12 months
        const futureDate = new Date(firstSelectableDate.getFullYear(), firstSelectableDate.getMonth() + m, firstSelectableDate.getDate());
        
        if (futureDate > new Date()) { // Only enable future dates
  ...
  const confirmSetBudget = async() => {
    ...
    const response = await BudgetController(userEmail, "monthly", numericLimitAmount, alterDate)
  ...
  const confirmationMessage = useMemo(() => {
    ...
    return `Are you sure you want to set a monthly budget of ₦${numericAmount.toLocaleString()} until ${alterDate.toLocaleDateString()}? This cannot be changed for ${dayDifference} days.`;
  ...
  return (
    <SafeAreaView className="flex-1 bg-gray-50">
      ...
      <View className="flex-row items-center px-4 py-8 bg-gray-50">
        ...
        <Text className="flex-1 text-2xl font-monasans-bold text-center color-[#333]">Set Monthly Budget</Text>
        <View style={{ width: 24 }} />
      </View>
